...
class Renderer {
  ...
  draw({ canvasHeight, canvasWidth, scrollTop, firstVisibleRow, lines, selections }) {
    ...
    const selectionSolidCount = this.populateSelectionSolidInstances(scrollTop, selections, xPositions);
  ...
  populateSelectionSolidInstances(selections, xPositions) {
    ...
    for (var i = 0; i < selections.length; i++) {
      ...
      if (comparePoints(selection.start, selection.end) !== 0) {
        ...
        const startX = xPositions.get(pointToKey(selection.start))
        const endX = xPositions.get(pointToKey(selection.end))
        ...
        if (rowSpan === 0) {
          this.updateSelectionSolidInstance(
            ...
            selectionSolidCount++,
            startX,
            selection.start.row * this.style.computedLineHeight,
            endX - startX,
            this.style.computedLineHeight,
            selectionColor
          );
        } else {
          ...
          if (rowSpan > 1) {
            this.updateSelectionSolidInstance(
              ...
              selectionSolidCount++,
              0,
              (selection.start.row + 1) * this.style.computedLineHeight,
              canvasWidth,
              (rowSpan - 1) * this.style.computedLineHeight,
              selectionColor
            );
          }
          ...
          this.updateSelectionSolidInstance(
            ...
            selectionSolidCount++,
            0,
            (selection.end.row - 1) * this.style.computedLineHeight,
            endX,
            this.style.computedLineHeight,
            selectionColor
          );
